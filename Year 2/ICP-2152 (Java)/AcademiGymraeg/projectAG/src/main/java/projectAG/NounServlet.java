	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!noun.equals("") && !translation.equals("")) {
			...
			if (action.equals("add")) {
				...
				//update sql database with new info
			...
			//--attempt to see if the nouns list required updating to view the new list with new info
			//but as it turns out, even in sql database the info is not getting updated
			try {
				nouns = db.getTable();
			} catch (SQLException e) {
			}
		} else {
			//if one or both of the translation and noun fields were empty in editor then
			//nothing will change in the database and a message posted to the user on return to NounServlet get method
			fail = true;
		}

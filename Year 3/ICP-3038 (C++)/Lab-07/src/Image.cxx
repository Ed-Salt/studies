...
double Image::getRMSE(const Image& anImage) const
{
    ...
    if (wid == this->getWidth() && hei == this->getHeight()) {
        for (size_t j = 0; j < hei; ++j)
        {
            ...
            {
                temp = sqrt((1/(wid*hei))*((j*2)*(i*2))*((anImage(i, j) - (*this)(i, j))*(anImage(i, j) - (*this)(i, j))));
            }
        }
        return temp;
    }
    ...
    if (wid == this->getWidth() && hei == this->getHeight()) {
        for (float j = 0; j < hei; ++j)
        {
            for (float i = 0; i < wid; ++i)
            {
                ...
                temp = ((float)1.0 / (wid * hei)) *((j * (float)2.0) * (i * (float)2.0)) * (((anImage(i, j) - anImage.m_average_pixel_value)*((*this)(i, j) - this->m_average_pixel_value))/(anImage.m_stddev_pixel_value*this->m_stddev_pixel_value));
            }
        }
        return temp;
    }
